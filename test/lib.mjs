...
export const ffmpeg = (args) =>
  execFileAsync(FFMPEG, ['-hide_banner', '-loglevel', 'error', ...args], { maxBuffer: MAX_BUFFER });

export const ffprobe = async (args) =>
  (await execFileAsync(FFPROBE, ['-v', 'error', ...args], { maxBuffer: MAX_BUFFER })).stdout.trim();

/** [pts, dts] of every packet in a stream. */
export async function packets(file, stream = '0') {
  const out = await ffprobe(['-select_streams', stream, '-show_entries', 'packet=pts,dts', '-of', 'csv=p=0', file]);
  return out ? out.split(/\r?\n/).map((line) => line.split(',').map(Number)) : [];
}

export async function packetCount(file, stream) {
  return Number(
    await ffprobe(['-select_streams', String(stream), '-count_packets', '-show_entries', 'stream=nb_read_packets', '-of', 'csv=p=0', file]),
  );
}
...
const childBox = (buf, parent, type) =>
  boxes(buf, parent.start + 8, parent.start + parent.size).find((box) => box.type === type);
...
export function trackTimescale(init) {
  ...
  const mdhd = childBox(init, childBox(init, childBox(init, moov, 'trak'), 'mdia'), 'mdhd');
  return init.readUInt32BE(mdhd.start + 8 + (init[mdhd.start + 8] === 1 ? 20 : 12));
}
...
export function directorySize(dir) {
  try {
    return readdirSync(dir, { recursive: true, withFileTypes: true })
      ...
      .reduce((total, entry) => total + statSync(path.join(entry.parentPath, entry.name)).size, 0);
  } catch {
    return 0;
  }
}
...
export function startServer({ name, port, cacheDir, env = {} }) {
  ...
  const log = createWriteStream(path.join(workDir, 'logs', `${name}.log`), { flags: 'a' });
  const child = spawn(process.execPath, ['dist/index.js'], {
    cwd: root,
    env: { ...process.env, PORT: String(port), FFMPEG_PATH: FFMPEG, CACHE_DIR: cacheDir, LOG_LEVEL: 'debug', ...env },
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) => reject(new Error(`server ${name} exited with ${code}; see test/.work/logs/${name}.log`)));
  });
}
