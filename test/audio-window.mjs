// Converted audio is read from a tight window first: only the clusters its
// timestamps fall in. That has to be both true and safe - tight reads succeed
// on normally muxed files, and a file that stores audio further out is caught
// and read generously, producing the very same segment.
...
import { FFMPEG, check, failureCount, fixturesDir, section, workDir } from './lib.mjs';

const out = path.join(workDir, 'audio-window');
await rm(out, { recursive: true, force: true });
await mkdir(out, { recursive: true });

/** A source that counts the bytes a render asks for. */
...
for (const [name, target] of [
  ...
  section(`audio window - ${name} at ${target}s, ${converted.length} converted tracks`);
  ...
  for (const rendition of converted) {
    ...
    for (let n = 0; n < count; n++) {
      const segment = path.join(out, `${name}-${rendition.track.number}-${n}.m4s`);
    ...
    // The same renders with only the generous read, for comparison.
    ...
    let identical = true;
    for (let n = 0; n < count; n++) {
      const segment = path.join(out, `${name}-${rendition.track.number}-${n}-wide.m4s`);
      await wide.writeSegment({ index, source: wideSource, rendition }, n, segment);
      identical &&= (await readFile(segment)).equals(await readFile(rendered[n]));
    }
    check(identical, `${label}: tight and generous reads produce identical segments`);
    check(
  ...
  // A file whose audio is stored earlier than its clusters: pretend the keyframe
  // clusters just before a segment start later than they do. The tight read
  // then begins past blocks it needs.
  ...
  // Short of where the generous read starts (2s of slack plus the audio margin
  // before the segment), so only the tight read is misled.
  ...
  await remuxer.writeSegment({ index: misplaced, source: local, rendition }, n, recovered);

  check(moved > 0, `${name}: the test really misplaces a keyframe cluster`, `${moved} moved`);
  check(
