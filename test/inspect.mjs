...
// Usage: inspect.mjs <magnet|path> [file index or -] [segments] [--remux]
...
if (target.startsWith('magnet:')) {
  ...
  // deselect keeps the client from fetching anything the scan does not ask
  // for, which is what makes the byte total at the end mean something.
...
// Tallies blocks straight out of a byte range, without the project's
// slicer, so a slice can be compared against what it really contains.
async function scan(start, end) {
  ...
  while (await need(12)) {
    ...
    if (el.id === Id.Timestamp) {
      let v = 0;
      for (let i = el.headerLength; i < body.length; i++) {
        v = v * 256 + body[i];
      }
      clusterTs = v;
      continue;
...
for (const n of segments) {
  ...
  // Mirrors the window the remuxer asks for: 1024-sample AAC frame grid
  // plus padding, so the read range printed here is the real one.
...
// Unreffed backstop for a destroy callback that never fires.
