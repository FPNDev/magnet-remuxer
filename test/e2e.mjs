// End to end: seeds a fixture from a local peer and drives the real HTTP
// server the way players do - sequential playback, seeking, several clients at
// once, a restart, and switching to a track whose pieces were already evicted.
...
// ---- server A: sequential playback with a small piece budget ---------------
...
for (let n = 0; n < videoPlaylist.segments.length; n++) {
  // A player asks for the renditions of one time slot together.
...
for (const url of otherAudio.segments) {
  ...
  if (res.status !== 200) otherFailures++;
