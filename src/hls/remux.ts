...
export class Remuxer {
   ...
   * Needs only the file header, no media data.
   */
  async writeInit(
    index: MediaIndex,
    rendition: Rendition,
    ...
    await runFfmpeg(this.options.ffmpegPath, {
      args: plan(index, rendition, 0).args,
      // The demuxer only reports stream parameters once it reaches a cluster.
      input: [Buffer.concat([trackHeader(index, rendition), EMPTY_CLUSTER])],
      output: splitter,
      signal,
  ...
  async writeSegment(target: RemuxTarget, n: number, outPath: string): Promise<void> {
...
function plan(index: MediaIndex, rendition: Rendition, n: number): Plan {
  const keyframeSlices = [keyframeSlice(index, n, true), keyframeSlice(index, n, false)];
  ...
  switch (rendition.type) {
    ...
    case 'subtitle':
      return { slices: keyframeSlices, args: [...INPUT_ARGS, ...WEBVTT_OUTPUT_ARGS] };
  }
}
 ...
 */
function aacPlan(index: MediaIndex, rendition: AudioRendition, n: number): Plan {
  ...
  const drop = `lt(pts\\,${start})` + (end === null ? '' : `+gte(pts\\,${end})`);
  const channels = Math.min(2, rendition.track.channels ?? 2);
...
function isComplete(index: MediaIndex, slice: SliceTarget, result: SliceResult): boolean {
  ...
  if (range.to === null || result.reachedEnd || !result.sourceEnded) return true;
